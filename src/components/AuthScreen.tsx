...
export const AuthScreen: React.FC<AuthScreenProps> = ({ lang, onToggleLang }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 flex flex-col justify-between relative overflow-hidden">
      ...
      <header className="relative z-10 p-4 sm:p-6 max-w-7xl mx-auto w-full flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg border border-emerald-500/40 bg-emerald-950/60 hover:bg-emerald-900/80 text-emerald-300 transition-colors shadow-xs"
            title="Configure Gemini Key & Supabase"
          >
            <Key className="w-3.5 h-3.5 text-emerald-400" />
            <span className="hidden sm:inline">{isFr ? 'Clé API & Cloud' : 'API Key & Cloud'}</span>
          </button>
          ...
          <div className="p-6 sm:p-8 space-y-4 max-h-[75vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5">
              ...
              <div className="pt-2">
                <button
                  type="button"
                  onClick={() => loginAsGuest('Guest Teacher')}
                  className="w-full py-2.5 px-3 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2 border border-slate-200"
                >
                  <UserCheck className="w-4 h-4 text-emerald-600" />
                  <span>{isFr ? 'Accès Immédiat Démo / Mode Invité (Sans Compte)' : 'Continue as Demo Teacher (Quick Guest Mode)'}</span>
                </button>
              </div>
      ...
      {/* API Key and Supabase Config Modal */}
      <ApiKeyModal
