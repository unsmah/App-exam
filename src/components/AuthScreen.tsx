...
export const AuthScreen: React.FC<AuthScreenProps> = ({ lang, onToggleLang }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.code === 'auth/operation-not-allowed') {
        msg = isFr
          ? 'L\'authentification par e-mail/mot de passe n\'est pas activée sur votre projet Firebase. Vous pouvez continuer immédiatement en créant un compte local sur cet appareil.'
          : 'Email/Password sign-in is not enabled in your Firebase project console. You can continue immediately by creating a local teacher account on this device.';
        canFallback = true;
      } else if (err.code === 'auth/network-request-failed' || err.code === 'auth/internal-error') {
        msg = isFr
          ? 'Impossible de joindre les serveurs Firebase (hors-ligne ou réseau restreint). Vous pouvez basculer en mode compte local.'
          : 'Unable to reach Firebase servers (offline or restricted network). You can switch to local teacher mode.';
        canFallback = true;
      } else if (
  ...
  const handleCreateLocalDirect = () => {
    ...
    createLocalAccount(finalEmail, finalName, {
      ...
      wilaya,
      commune: commune.trim() || 'Alger Centre',
      academicYear,
      teachingLevels
    });
  };
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      setError(
        isFr
          ? 'La connexion Google n\'est pas disponible dans cette vue WebView. Veuillez utiliser l\'e-mail ou le mode Local.'
          : 'Google sign-in is not supported in standalone WebView. Please use Email or Local Teacher Mode.'
      );
      setErrorDetails({ code: err.code, canFallbackLocal: true });
    } finally {
      setLoading(false);
    }
  };
      ...
      <main className="relative z-10 flex-1 flex items-center justify-center p-4 sm:p-6">
        <div className="w-full max-w-xl bg-white text-slate-900 rounded-2xl shadow-2xl border border-slate-200/80 overflow-hidden my-4">
          ...
          <div className="flex border-b border-slate-200 bg-slate-50 text-xs font-semibold">
            ...
            >
              {isFr ? 'Créer un Compte' : 'Register Account'}
            </button>
            {mode === 'reset' && (
              <button
                type="button"
          ...
          <div className="p-6 sm:p-8 space-y-4 max-h-[75vh] overflow-y-auto">
            {error && (
              <div className="p-3.5 bg-amber-50 border border-amber-300 rounded-xl text-amber-900 text-xs space-y-2.5 animate-fadeIn">
                <div className="flex items-start gap-2.5">
                  <AlertCircle className="w-4 h-4 shrink-0 text-amber-700 mt-0.5" />
                  <div className="space-y-1">
                    <p className="font-semibold">{error}</p>
                    {errorDetails?.code && (
                      <p className="text-[10px] text-amber-700 font-mono">Code: {errorDetails.code}</p>
                    )}
                  </div>
                </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5">
              ...
              {mode === 'reset' && (
                <div className="text-center pt-2">
                  <button
                    type="button"
                    onClick={() => { setMode('login'); setError(null); setErrorDetails(null); }}
            ...
            <div className="pt-3 border-t border-slate-200">
              ...
              >
                ...
                <span>
                  {isFr
                    ? '⚡ Démarrage Rapide (Mode Enseignant Local / APK)'
                    : '⚡ Quick Start (Local Teacher / Standalone APK Mode)'}
                </span>
              </button>
              <p className="text-[10px] text-slate-500 text-center mt-1.5">
                {isFr
                  ? 'Permet d\'utiliser immédiatement toutes les fonctionnalités (IA, BEM, Sujets, Question Bank) sans configuration de serveur.'
                  : 'Allows full instant access to all exam generation, question bank, and DOCX tools locally.'}
              </p>
            </div>
            ...
            <div className="pt-3 border-t border-slate-100 grid grid-cols-3 gap-2 text-center">
              ...
              <div className="p-2 rounded-lg bg-slate-50 border border-slate-100">
                ...
                <div className="text-[10px] font-bold text-slate-700">Local & Cloud</div>
                <div className="text-[9px] text-slate-400">Full Offline Support</div>
              </div>
            </div>
          </div>
        </div>
